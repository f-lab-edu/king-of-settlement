...
public class UserController {
	...
	@PostMapping("/logout")
	public ResponseEntity<CommonResponse<?>> logout(final HttpServletRequest request) {
		userService.logout(request);
		return ResponseEntity.ok().body(CommonResponse.success(CommonStatusCode.Succeed));
	}

	@PutMapping("/{userId}")
	public ResponseEntity<CommonResponse<?>> updateUser(@PathVariable(name = "userId") Long userId,
